...
class ComponentLoader {
    ...
    // Caminho fixo onde seus componentes realmente estão
    const componentsDir = "/frontend/public/components/";

    // Quebra o path atual em partes (ex: ["", "frontend", "public", "navigation", "candidatos.html"])
    const currentParts = current.split("/").filter(Boolean);
    const targetParts = componentsDir.split("/").filter(Boolean);

    // Remove o arquivo do final
    if (currentParts[currentParts.length - 1].includes(".")) {
        currentParts.pop();
    }

    // Encontra onde começam a divergir
    let i = 0;
    while (i < currentParts.length && i < targetParts.length && currentParts[i] === targetParts[i]) {
        i++;
    }

    // Quantos níveis precisamos subir
    const goUpLevels = currentParts.length - i;

    // Monta "../" para cada nível
    const up = "../".repeat(goUpLevels);

    // Monta a parte restante do caminho até components/
    const down = targetParts.slice(i).join("/") + "/";

    const finalPath = up + down;

    console.log(`📂 getBasePath() => ${finalPath}`);
